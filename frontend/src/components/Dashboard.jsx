    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12 }}>
          {[
            { label: "Total clientes",   value: clients.length,  color: "var(--gold)" },
            { label: "Particulares",     value: particulares,    color: "var(--gold)" },
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12 }}>
          {[
            { label: "Total siniestros", value: claims.length,          color: "var(--gold)" },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {byStage.map(s => (
              <div key={s.stage} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 110, fontSize: 14, color: "var(--textSub)", fontFamily: "Plus Jakarta Sans, sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.stage}</div>
                ...
                <div style={{ width: 90, fontSize: 14, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif", textAlign: "right" }}>{s.prima ? `${s.prima.toLocaleString("es-ES")} €` : "—"}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {byRamo.map((r, i) => (
                  <div key={r.ramo} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 110, fontSize: 14, color: "var(--textSub)", fontFamily: "Plus Jakarta Sans, sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.ramo}</div>
                    <div style={{ flex: 1, background: "var(--lift)", borderRadius: 4, height: 8, overflow: "hidden" }}>
                    ...
                    <div style={{ width: 90, fontSize: 14, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif", textAlign: "right" }}>{r.prima ? `${r.prima.toLocaleString("es-ES")} €` : "—"}</div>
                  </div>
                ))}
              </div>
          }
            ...
            : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {byAseguradora.map((a, i) => (
                  <div key={a.aseguradora} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 110, fontSize: 14, color: "var(--textSub)", fontFamily: "Plus Jakarta Sans, sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.aseguradora}</div>
                    <div style={{ flex: 1, background: "var(--lift)", borderRadius: 4, height: 8, overflow: "hidden" }}>
                    ...
                    <div style={{ width: 20, fontSize: 14, color: "var(--text)", fontFamily: "Plus Jakarta Sans, sans-serif", textAlign: "right" }}>{a.count}</div>
                    <div style={{ width: 90, fontSize: 14, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif", textAlign: "right" }}>{a.prima ? `${a.prima.toLocaleString("es-ES")} €` : "—"}</div>
                  </div>
                ))}
              </div>
          }
        ...
        <div style={S.card}>
          ...
          {sinByRamo.length === 0
            ? <div style={{ color: "var(--mute)", fontSize: 12 }}>Sin siniestros</div>
            : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {sinByRamo.map((r, i) => (
                  <div key={r.ramo} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 110, fontSize: 14, color: "var(--textSub)", fontFamily: "Plus Jakarta Sans, sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.ramo}</div>
                    ...
                    <div style={{ width: 20, fontSize: 14, color: "var(--text)", fontFamily: "Plus Jakarta Sans, sans-serif", textAlign: "right" }}>{r.total}</div>
      ...
      <div style={S.card}>
        ...
        {topClientes.length === 0
          ? <div style={{ color: "var(--mute)", fontSize: 13, textAlign: "center", padding: "2rem" }}>Sin datos</div>
          : topClientes.map(c => (
              <div key={c.id} onClick={() => onNavigate("client_detail", c.id)}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, color: "var(--text)", fontFamily: "Plus Jakarta Sans, sans-serif", fontWeight: 500 }}>{c.name}</div>
                  <div style={{ fontSize: 13, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif", marginTop: 2 }}>{c.numPolizas} póliza{c.numPolizas !== 1 ? "s" : ""} · {c.tipo}</div>
                  <div style={{ marginTop: 6, background: "var(--lift)", borderRadius: 4, height: 6, overflow: "hidden" }}>
                    <div style={{ height: "100%", borderRadius: 4, width: `${(c.prima / maxPrima) * 100}%`, background: "var(--gold)", transition: "width 0.4s" }} />
                  </div>
                </div>
                <div style={{ fontSize: 15, color: "var(--gold)", fontFamily: "Plus Jakarta Sans, sans-serif", fontWeight: 600, whiteSpace: "nowrap" }}>
        ...
        {clientesRecientes.length === 0
          ...
          : clientesRecientes.map(c => {
              ...
              return (
                <div key={c.id} onClick={() => onNavigate("client_detail", c.id)}
                  style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
                    padding: "10px 0", borderBottom: "0.5px solid var(--border)", cursor: "pointer", flexWrap: "wrap", gap: 8 }}>
                  <div>
...
function KpiCard({ label, value, color }) {
  return (
    <div style={{ background: "var(--card)", border: "0.5px solid var(--border)", borderRadius: 8, padding: "16px 18px" }}>
      <div style={{ fontSize: 12, letterSpacing: "0.12em", color: "var(--mute)", textTransform: "uppercase", fontFamily: "Plus Jakarta Sans, sans-serif", marginBottom: 8 }}>{label}</div>
...
const S = {
  ...
  sectionLabel:{ fontSize: 12, letterSpacing: "0.15em", color: "var(--mute)", textTransform: "uppercase", fontFamily: "Plus Jakarta Sans, sans-serif", marginBottom: 10 },
  card:        { background: "var(--card)", border: "0.5px solid var(--border)", borderRadius: 8, padding: 20 },
  cardLabel:   { fontSize: 13, letterSpacing: "0.12em", color: "var(--mute)", textTransform: "uppercase", fontFamily: "Plus Jakarta Sans, sans-serif", marginBottom: 16 },
};
