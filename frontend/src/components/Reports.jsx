...
const prioridadColor = (p) =>
  p === "Alta" ? "#E08080" : p === "Media" ? "#C9A870" : "var(--mute)";

const estadoColor = (e) => {
  if (!e) return "var(--mute)";
  const m = { "Abierto": "#E08080", "En gestión": "#C9A870", "Pendiente documentación": "#8B9AE0", "Resuelto": "#6BBF8A", "Cerrado": "var(--mute)" };
  return m[e] || "var(--mute)";
};

export default function Reports({ clients, policies, claims, currentUser }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        ...
        {!isAdmin && (
          <div style={{ fontSize: 13, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif", marginTop: 4 }}>
            Actividad de <span style={{ color: "var(--gold)", fontWeight: 600 }}>{currentUser?.name}</span>
          </div>
        )}
      </div>
      ...
      <div style={{ background: "var(--card)", border: "0.5px solid var(--border)", borderRadius: 8, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {selectedGestiones.size > 0 && (
              <span style={{ fontSize: 11, color: "var(--gold)", fontFamily: "Plus Jakarta Sans, sans-serif" }}>
                {selectedGestiones.size} seleccionada{selectedGestiones.size > 1 ? "s" : ""}
              </span>
            )}
            {gestiones.length > 0 && (
              <span style={{ fontSize: 12, fontWeight: 700, background: "var(--goldDim, #C9A87022)",
                color: "var(--gold)", padding: "2px 10px", borderRadius: 12,
        ...
        {gestiones.length === 0
          ...
          : <>
              {selectedGestiones.size === 0 && (
                <div style={{ fontSize: 11, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif",
                ...
                return (
                  <div key={i} onClick={() => toggleGestion(i)}
                    ...
                    <input type="checkbox" checked={checked} onChange={() => toggleGestion(i)}
                      ...
                      style={{ width: 15, height: 15, accentColor: "var(--gold)", cursor: "pointer", flexShrink: 0, marginTop: 4 }} />
                    <div style={S.dot} />
                    <div style={{ flex: 1 }}>
      ...
      {/* ── RENOVACIONES ── */}
      <PreviewSection title={`Renovaciones pendientes a ${fecha}`} count={renovaciones.length} emptyMsg="✓ Sin renovaciones pendientes">
      ...
      {/* ── SINIESTROS ── */}
      <PreviewSection title="Siniestros en curso" count={siniestrosAbiertos.length} emptyMsg="✓ Sin siniestros abiertos">
      ...
      {/* ── TAREAS PENDIENTES ── */}
      <PreviewSection title="Tareas pendientes" count={tareasPendientes.length} emptyMsg="✓ Sin tareas pendientes">
          ...
          return (
            <div key={i} style={{ ...S.row, alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={S.meta}>
                  {t.cliente !== "—" && <span>{t.cliente} · </span>}
                  <span style={{ color: vencida ? "#E08080" : "var(--mute)" }}>Vence: {t.vencimiento}</span>
                  {t.asignado !== "—" && <span> · {t.asignado}</span>}
                </div>
                {t.descripcion !== "—" && <div style={{ ...S.desc, marginTop: 4 }}>{t.descripcion}</div>}
              </div>
...
function PreviewSection({ title, count, emptyMsg, children }) {
  return (
    <div style={{ background: "var(--card)", border: "0.5px solid var(--border)", borderRadius: 8, padding: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 13, letterSpacing: "0.12em", color: "var(--mute)", textTransform: "uppercase",
...
const S = {
  ...
  row:     { display: "flex", gap: 12, padding: "10px 0", borderBottom: "0.5px solid var(--border)" },
  dot:     { width: 6, height: 6, borderRadius: "50%", background: "var(--gold)", flexShrink: 0, marginTop: 6 },
  nombre:  { fontSize: 15, fontWeight: 600, color: "var(--gold)", fontFamily: "Plus Jakarta Sans, sans-serif" },
  meta:    { fontSize: 13, color: "var(--mute)", fontFamily: "Plus Jakarta Sans, sans-serif", marginTop: 2 },
  desc:    { fontSize: 15, color: "var(--text)", fontFamily: "Plus Jakarta Sans, sans-serif" },
};
